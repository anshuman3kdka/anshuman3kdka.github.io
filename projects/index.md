...
<section class="section category-browse" aria-label="Projects entries">
  ...
  {% for project in project_items %}
    {% assign item_publish_time = project.publish_date | date: '%s' %}
    {% assign item_is_public = true %}
    {% if project.publish_date and project.publish_date != '' and item_publish_time > current_time %}
  ...
  <div class="category-browse-summary tactile-deboss">
    ...
    <p class="category-browse-summary__count">{{ public_item_count }} {% if public_item_count == 1 %}project{% else %}projects{% endif %}</p>
  </div>
  ...
  <div class="category-browse-list">
    ...
    {% assign item_publish_time = project.publish_date | date: '%s' %}
    {% unless project.publish_date and project.publish_date != '' and item_publish_time > current_time %}
    ...
    <article class="category-browse-item tactile-card">
      <a class="category-browse-item__link" href="{{ project.url | relative_url | escape }}">
        <span class="category-browse-item__meta">{% if project.eyebrow %}{{ project.eyebrow | escape }}{% else %}Projects{% endif %}</span>
