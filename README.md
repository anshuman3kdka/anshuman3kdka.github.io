...
This repository hosts a static writing portfolio with a generated random-read index.
...
## Preventing `random-read.json` merge conflicts

`random-read.json` is generated output, so Git line-by-line merges often conflict when two branches both regenerate it.
...
Then commit the refreshed `random-read.json` if it changed.
...
It regenerates `random-read.json` and commits any updates automatically on `main`.
